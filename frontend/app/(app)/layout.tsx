  ...
  return (
    <div className="flex bg-background" style={{ height: "100dvh" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 border-t bg-background/95 backdrop-blur-sm flex">
          {nav.map(({ href, icon: Icon, label }) => (
            <Link
              key={href}
              href={href}
            ...
            >
              <span className={`relative flex items-center justify-center w-8 h-8 rounded-xl transition-all ${pathname === href ? "bg-gradient-brand-short" : ""}`}>
                <Icon size={18} className={pathname === href ? "text-white" : ""} />
                {href === "/chats" && totalUnread > 0 && (
                  <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-0.5 bg-gradient-brand-short text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                    {totalUnread > 9 ? "9+" : totalUnread}
                  </span>
                )}
              </span>
